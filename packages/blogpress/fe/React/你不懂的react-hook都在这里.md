...
- 父组件创建并且导出`context:export const MyContext = createContext()`
...
hooks 让函数组件拥有了内部状态，生命周期，使用 hook 让代码更加简洁，自定义 hooks 方便了对逻辑的复用，并且摆脱了 class 组件的 this 问题，但是在使用 hooks 时会产生一些闭包问题，需要仔细使用。
